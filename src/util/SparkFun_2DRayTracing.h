...
public:
	...
	bool contains(sf2drt_coordinate_t p)
	{
		...
		sf2drt_extent_t max_x = (*(_p_points)).x;
		sf2drt_extent_t max_y = (*(_p_points)).x;
		sf2drt_extent_t min_x = (*(_p_points)).y;
		sf2drt_extent_t min_y = (*(_p_points)).y;

		for(sf2drt_max_qty_t indi = 1; indi < _num_sides; indi++)
		{
			max_x = max(max_x, (*(_p_points + indi)).x);
			min_x = min(min_x, (*(_p_points + indi)).x);
			max_y = max(max_y, (*(_p_points + indi)).y);
			min_y = min(min_y, (*(_p_points + indi)).y);
		}
